            ...
            try {
                ...
                ticker.innerHTML = data.map(item => `
                    <div class="pulse-item">
                        ...
                        <span class="${item.trend.includes('+') ? 'pulse-trend-up' : 'pulse-trend-down'}">${item.trend}</span>
                    </div>
                `).join('') + data.map(item => `
                    <div class="pulse-item">
                        <span style="opacity:0.6;">${item.label}:</span>
        ...
        function showCompareBar() {
            ...
            if (!bar) {
                ...
                bar.id = 'compareBar';
                bar.style = "position:fixed; bottom:2rem; left:50%; transform:translateX(-50%); background:var(--accent-primary); color:white; padding:1rem 2.5rem; border-radius:100px; display:flex; gap:2rem; align-items:center; z-index:4000; box-shadow:0 20px 50px rgba(0,0,0,0.5); font-weight:900;";
                    ...
                    <div style="display:flex; justify-content: space-between; align-items: center; margin-top: auto; padding-top: 2rem;">
                        <div style="display:flex; gap:2rem; align-items:center;">
                            <div>
                                ...
                                <div class="price-val" style="font-size: 2.5rem;">$${p.price_min.toLocaleString()}</div>
                            </div>
                            <div style="background:rgba(255,255,255,0.05); padding:0.8rem; border-radius:15px; display:flex; flex-direction:column; gap:0.4rem; border:1px solid rgba(255,255,255,0.1);">
                                <label style="font-size:0.65rem; color:var(--text-muted); font-weight:800; text-transform:uppercase;">Order Qty</label>
                                <input type="number" id="orderQty" value="1" min="1" style="background:transparent; border:none; border-bottom:2px solid var(--accent-primary); color:white; font-size:1.4rem; font-weight:900; width:70px; outline:none; text-align:center;">
                            </div>
                        </div>
                        <button class="btn-add-cart" onclick="addToCart(${p.id})" style="padding: 1.2rem 2.5rem; font-size: 1.1rem; border-radius:15px;">⚡ DEPLOY TO CART</button>
                    </div>
                </div>
            `;
            modal.classList.add('active');
